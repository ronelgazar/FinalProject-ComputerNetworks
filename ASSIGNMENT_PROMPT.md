...
- Flags: SYN=0x01 ACK=0x02 FIN=0x04 DATA=0x08 RST=0x10 PING=0x20
- CRC-16/CCITT-FALSE checksum (poly=0x1021, init=0xFFFF) over full header+payload
- Sliding window (size 8), cumulative ACK, retransmit timer 500ms, max 5 retries
- 3-way handshake (SYN→SYN+ACK→ACK) and graceful close (FIN+ACK)
...
```
 0       4       8    9       11      13      14
 +-------+-------+----+-------+-------+-------+------...
 | seq   | ack   |flg |window | len   | crc16 | payload
 +-------+-------+----+-------+-------+-------+------...
   4B      4B     1B    2B      2B      2B      len bytes
```
Total overhead: 14 bytes (vs TCP minimum 20 bytes — 30% smaller)

**RUDP vs TCP header overhead:**
- RUDP: 14 bytes fixed (no OS stack, custom flow control)
- TCP: ≥20 bytes + OS buffer management + Nagle algorithm
